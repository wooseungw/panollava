...
class VLMModule(pl.LightningModule):
    ...
    def __init__(self, *, stage: str, model_config: ModelConfig, lr: float,
        ...
        # resampler_config 세부 설정 추출 (평가 시 차원 불일치 방지)
        resampler_cfg = getattr(model_config, 'resampler_config', None)
        resampler_details = {}
        if resampler_cfg is not None:
            # 공통 필드
            resampler_details["resampler_depth"] = getattr(resampler_cfg, 'depth', None)
            resampler_details["resampler_hidden_dim"] = getattr(resampler_cfg, 'hidden_dim', None)
            resampler_details["resampler_dropout"] = getattr(resampler_cfg, 'dropout', None)
            resampler_details["resampler_use_ln"] = getattr(resampler_cfg, 'use_ln', None)
            resampler_details["resampler_num_views"] = getattr(resampler_cfg, 'num_views', None)
            # BiMamba 전용 필드
            resampler_details["resampler_d_state"] = getattr(resampler_cfg, 'd_state', None)
            resampler_details["resampler_d_conv"] = getattr(resampler_cfg, 'd_conv', None)
            resampler_details["resampler_expand"] = getattr(resampler_cfg, 'expand', None)
            resampler_details["resampler_norm_first"] = getattr(resampler_cfg, 'norm_first', None)
            resampler_details["resampler_enable_cross_view"] = getattr(resampler_cfg, 'enable_cross_view', None)
            # Perceiver 전용 필드
            resampler_details["resampler_num_latents"] = getattr(resampler_cfg, 'num_latents', None)
            resampler_details["resampler_heads"] = getattr(resampler_cfg, 'heads', None)
            # MLP 전용 필드
            resampler_details["resampler_pool_tokens"] = getattr(resampler_cfg, 'pool_tokens', None)
            resampler_details["resampler_pool_type"] = getattr(resampler_cfg, 'pool_type', None)
            # QFormer 전용 필드
            resampler_details["resampler_num_query_tokens"] = getattr(resampler_cfg, 'num_query_tokens', None)
            resampler_details["resampler_num_hidden_layers"] = getattr(resampler_cfg, 'num_hidden_layers', None)
            resampler_details["resampler_num_attention_heads"] = getattr(resampler_cfg, 'num_attention_heads', None)
            # None 값 제거
            resampler_details = {k: v for k, v in resampler_details.items() if v is not None}
        ...
        checkpoint_metadata = {
            ...
            # resampler_config 세부 설정 (평가 시 차원 불일치 방지)
            **resampler_details,
...
def build_datamodule(cfg: Dict[str, Any], stage_cfg: Dict[str, Any]) -> VLMDataModule:
    ...
    dm = VLMDataModule(
        ...
        use_spherical_inputs=ip.get("use_spherical_inputs", False),
        spherical_column=ip.get("spherical_column", "hp_path"),
        spherical_key=ip.get("spherical_key", "hp_img"),
        spherical_dtype=ip.get("spherical_dtype", "float32"),
