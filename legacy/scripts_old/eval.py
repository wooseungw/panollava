    ...
    **model_kwargs
    ...
    if config_data is not None:
        if isinstance(config_data, dict):
            # 만약 config_data가 전체 설정(Root) 구조라면 models 섹션만 추출
            if 'models' in config_data:
                config_obj = config_data['models']
            else:
...
def main():
    ...
    # ========== 일반 모드: Config 로딩 필요 ==========
    # config가 명시되지 않은 경우 체크포인트 디렉토리에서 config.yaml 자동 탐색
    # config가 명시되지 않은 경우 체크포인트 디렉토리에서 config.yaml 자동 탐색
    config_to_load = args.config
    if config_to_load is None and explicit_checkpoint_path:
        ckpt_dir = explicit_checkpoint_path.parent if explicit_checkpoint_path.is_file() else explicit_checkpoint_path
        candidates = [
        ...
        for cand in candidates:
            if cand.exists():
                config_to_load = str(cand)
                logger.info(f"🔍 체크포인트 디렉토리에서 config 자동 감지: {cand}")
                break
    
    global_config = load_global_config(config_to_load)
    max_samples_cli = args.max_samples if args.max_samples and args.max_samples > 0 else None
