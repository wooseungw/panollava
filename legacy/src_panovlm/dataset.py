...
class BaseChatPanoDataset(Dataset):
    ...
    def __init__(
        ...
        self.has_instruction = 'instruction' in self.df.columns
        self.has_response = 'response' in self.df.columns
        
        # Instruction tuning 모드 확인
        self.is_instruction_tuning = self.has_instruction and self.has_response
        ...
        logger.info(f"Has instruction: {self.has_instruction}")
        logger.info(f"Has response: {self.has_response}")
        logger.info(f"Is instruction tuning: {self.is_instruction_tuning}")
    ...
    def _process_batch_with_text(self, pil: Image.Image, user_query: str, assistant_response: str = None,
                                 pixel_values: Optional[torch.Tensor] = None) -> dict:
    ...
    def _add_metadata(self, batch: dict, row: pd.Series, idx: int):
        ...
        if self.use_spherical_inputs and hasattr(row, self.spherical_column):
            batch["spherical_path"] = str(getattr(row, self.spherical_column))

    def _load_spherical_tensor(self, row: pd.Series, idx: int) -> torch.Tensor:
        if not hasattr(row, self.spherical_column):
            raise ValueError(f"Missing spherical column '{self.spherical_column}' in CSV")
        path = str(getattr(row, self.spherical_column))
        if not path:
            raise ValueError("Empty spherical path")
        if not os.path.exists(path):
            raise FileNotFoundError(f"Spherical input not found: {path}")

        if path.endswith(".npz"):
            data = np.load(path)
            if self.spherical_key not in data:
                raise KeyError(f"Key '{self.spherical_key}' not found in {path}")
            arr = data[self.spherical_key]
        else:
            arr = np.load(path)
        ...
        tensor = torch.tensor(arr, dtype=getattr(torch, self.spherical_dtype))
        return tensor
...
class ChatPanoDataset(BaseChatPanoDataset):
    ...
    def __init__(self,
                 ...
                 use_spherical_inputs: bool = False,
                 spherical_column: str = "hp_path",
                 spherical_key: str = "hp_img",
                 spherical_dtype: str = "float32"):
        ...
        self.use_spherical_inputs = use_spherical_inputs
        self.spherical_column = spherical_column
        self.spherical_key = spherical_key
        self.spherical_dtype = spherical_dtype
    ...
    def __getitem__(self, idx):
        ...
        try:
            ...
            pil = None
            pixel_values = None
            if self.use_spherical_inputs:
                pixel_values = self._load_spherical_tensor(row, idx)
            else:
                pil = self._load_image(row.url, idx)
            
            # 이미지 증강 적용 (훈련 모드에만)
            if self.color_augmentation is not None and pil is not None:
            ...
            if self.is_instruction_tuning:
                # Instruction tuning: instruction-response 쌍 사용
                user_query = str(row.instruction)
                assistant_response = str(row.response)
                batch = self._process_batch_with_text(pil, user_query, assistant_response, pixel_values=pixel_values)
            elif self.mode == "train" or (self.mode != "eval" and hasattr(row, 'annotation')):
                # 기존 학습 모드: query-annotation 쌍 사용
                ...
                # 평가/생성 모드: 사용자 쿼리만
                user_query = self._format_user_query(str(row.query))
                batch = self._process_batch_with_text(pil, user_query, None, pixel_values=pixel_values)
        ...
        try:
                ...
                if self.hparams.eval_mode:
                    ...
                    self.val_ds = ChatPanoDataset(self.hparams.csv_val,
                                                  ...
                                                  use_spherical_inputs=self.hparams.use_spherical_inputs,
                                                  spherical_column=self.hparams.spherical_column,
                                                  spherical_key=self.hparams.spherical_key,
                                                  spherical_dtype=self.hparams.spherical_dtype)
