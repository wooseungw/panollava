...
class PanoramaVLM(nn.Module):
    ...
    def __init__(self, config=None, **kwargs):
        ...
        vision_backbone_type = getattr(self.config, 'vision_backbone_type', 'hf')
        vision_backbone_kwargs = getattr(self.config, 'vision_backbone_kwargs', None)
        vision_input_key = getattr(self.config, 'vision_input_key', 'pixel_values')
        vision_output_key = getattr(self.config, 'vision_output_key', 'last_hidden_state')
        vision_forward_method = getattr(self.config, 'vision_forward_method', None)
        vision_hidden_size = getattr(self.config, 'vision_hidden_size', None)
        self.vision_backbone = VisionBackbone(
            vision_name=vision_name,
            use_vicreg_norm=use_vicreg_norm,
            backbone_type=vision_backbone_type,
            backbone_kwargs=vision_backbone_kwargs,
            input_key=vision_input_key,
            output_key=vision_output_key,
            forward_method=vision_forward_method,
        ...
        try:
            ...
            if user_model_config is not None:
                ...
                if detected_config:
                    ...
                    # 2. 하이퍼파라미터에서 설정 생성 (resampler 세부 설정 포함)
                    config_dict = {
                        ...
                        'overlap_ratio': hparams.get('overlap_ratio', hparams.get('model_overlap_ratio', 0.5)),
                        'max_text_length': hparams.get('max_text_length', 512),
                    }
                    # resampler 세부 설정 추가 (평가 시 차원 불일치 방지)
                    resampler_keys = [
                        'resampler_depth', 'resampler_hidden_dim', 'resampler_dropout', 
                        'resampler_use_ln', 'resampler_num_views',
                        'resampler_d_state', 'resampler_d_conv', 'resampler_expand',
                        'resampler_norm_first', 'resampler_enable_cross_view',
                        'resampler_num_latents', 'resampler_heads',
                        'resampler_pool_tokens', 'resampler_pool_type',
                        'resampler_num_query_tokens', 'resampler_num_hidden_layers', 'resampler_num_attention_heads'
                    ]
                    for rk in resampler_keys:
                        if rk in hparams and hparams[rk] is not None:
                            config_dict[rk] = hparams[rk]
            ...
            if isinstance(model_config, LegacyModelConfig) and isinstance(hparams, dict) and len(hparams) > 0:
                override_keys = [
                    ...
                    # resampler 세부 설정 (평가 시 차원 불일치 방지)
                    'resampler_depth', 'resampler_hidden_dim', 'resampler_dropout', 
                    'resampler_use_ln', 'resampler_num_views',
                    'resampler_d_state', 'resampler_d_conv', 'resampler_expand',
                    'resampler_norm_first', 'resampler_enable_cross_view',
