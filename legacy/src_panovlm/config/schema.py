...
class MLPResamplerConfig(ResamplerConfigBase):
    ...
    pool_tokens: Optional[int] = Field(default=None, gt=0, description="Optional token pooling length")
    pool_type: Literal["avg", "max"] = Field(default="avg", description="Pooling type for MLP resampler")
...
class BiMambaResamplerConfig(ResamplerConfigBase):
    ...
    hidden_dim: int = Field(default=768, gt=0, description="Hidden dimension")
    dropout: float = Field(default=0.1, ge=0.0, le=1.0, description="Dropout applied inside Mamba blocks")
...
class ModelConfig(BaseModel):
    ...
    vision_backbone_type: Literal["hf", "timm", "torchhub", "module"] = Field(
        default="hf",
        description="Vision backbone loader type"
    )
    vision_backbone_kwargs: Optional[Dict[str, Any]] = Field(
        default=None,
        description="Extra kwargs passed to the vision backbone loader"
    )
    vision_input_key: str = Field(default="pixel_values", description="Vision forward input key")
    vision_output_key: str = Field(default="last_hidden_state", description="Vision output attribute key")
    vision_forward_method: Optional[str] = Field(default=None, description="Vision forward method override")
    vision_hidden_size: Optional[int] = Field(default=None, description="Override vision hidden size")
    ...
    resampler_pool_tokens: Optional[int] = Field(default=None, description="Token pooling length for MLP resampler")
    resampler_pool_type: Optional[str] = Field(default=None, description="Token pooling type for MLP resampler")
    ...
    use_projection_positional_encoding: bool = Field(default=False, description="Use positional encoding in projection layer")
    projection_intermediate_size: Optional[int] = Field(default=None, description="Intermediate hidden size in projection MLP")
    pe_view_encoding_type: str = Field(default="sinusoidal", description="View encoding type for PE")
    pe_spatial_encoding_type: str = Field(default="sinusoidal", description="Spatial encoding type for PE")
    pe_enable_continuity: bool = Field(default=True, description="Enable continuity in PE")
    pe_temperature: float = Field(default=10000.0, description="Temperature for PE")
    pe_dropout: float = Field(default=0.0, ge=0.0, le=1.0, description="Dropout rate for PE")

    # VICReg projector 구성 (stage1 전용 헤드)
    use_vicreg_projector: bool = Field(default=True, description="Use a dedicated VICReg projector head")
    vicreg_projector_dim: Optional[int] = Field(default=None, description="Output dimension of VICReg projector (defaults to latent_dimension * 2)")
    vicreg_projector_depth: int = Field(default=2, gt=0, description="Number of layers inside the VICReg projector")
    vicreg_projector_hidden: Optional[int] = Field(default=None, description="Hidden dimension inside the VICReg projector")
    vicreg_projector_ln: bool = Field(default=True, description="Apply LayerNorm blocks inside the VICReg projector")
    ...
    def apply_resampler_defaults(cls, values):
        ...
        # resampler_config가 없으면 타입에 맞게 자동 생성
            ...
            if resampler_type == 'mlp':
                ...
                base_params['pool_tokens'] = values.get('resampler_pool_tokens', resampler_defaults.get('pool_tokens'))
                base_params['pool_type'] = values.get('resampler_pool_type', resampler_defaults.get('pool_type', 'avg'))
        ...
        if values.get('resampler_config') is not None:
            ...
            if isinstance(rc, MLPResamplerConfig):
                ...
                values['resampler_pool_tokens'] = rc.pool_tokens
                values['resampler_pool_type'] = rc.pool_type
    ...
    def get_model_kwargs(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'vision_backbone_kwargs': self.vision_backbone_kwargs,
            'vision_input_key': self.vision_input_key,
            'vision_output_key': self.vision_output_key,
            'vision_forward_method': self.vision_forward_method,
    ...
    def from_dict(cls, config_dict: Dict[str, Any]) -> 'ModelConfig':
        ...
        필드 이름 자동 매핑:
        - vision_model_name → vision_name (train.py 호환)
        - lm_model → language_model_name (legacy 호환)
        """
        # 복사본 생성 (원본 수정 방지)
        ...
        # 필드 이름 매핑 (aliases)
        field_aliases = {
            'vision_model_name': 'vision_name',  # train.py에서 사용하는 이름
            'lm_model': 'language_model_name',   # legacy config
            'lm_name': 'language_model_name',    # 또 다른 legacy
        }
...
class ImageProcessingConfig(BaseModel):
    ...
    use_spherical_inputs: bool = Field(default=False, description="Load spherical inputs from files instead of ERP images")
    spherical_column: str = Field(default="hp_path", description="CSV column name for spherical input paths")
    spherical_key: str = Field(default="hp_img", description="Key inside spherical npz files")
    spherical_dtype: str = Field(default="float32", description="Torch dtype for spherical inputs")
...
class TrainingConfig(BaseModel):
    ...
    wandb_project: Optional[str] = Field(default="panollava-training")
...
class ConfigManager:
    ...
    def _flatten_json_config(config_dict: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if "training" in config_dict:
            ...
            stage_cfgs = training.get("stage_configs") or {}
            vision_cfg = stage_cfgs.get("vision") or {}
            if isinstance(vision_cfg, dict):
                for key in ("vicreg_loss_weight", "vicreg_similarity_weight", "vicreg_variance_weight", "vicreg_covariance_weight"):
                    if key in vision_cfg:
                        flat[key] = vision_cfg[key]

        return flat

    @staticmethod
    def auto_detect_config(checkpoint_path: Union[str, Path]) -> Optional[ModelConfig]:
        """체크포인트 경로 주변에서 ModelConfig를 자동 탐색."""
        ...
        for config_path in candidates:
            if config_path.exists():
                try:
                    ...
                    warnings.warn(f"설정 파일 로딩 실패 ({config_path}): {exc}")
        return None
