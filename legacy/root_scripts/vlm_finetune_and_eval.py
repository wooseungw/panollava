...
class QwenVLAdapter(CausalAdapter):
    ...
    def load_model(self) -> torch.nn.Module:
        dtype = _resolve_dtype(self.model_cfg.dtype)
        # Qwen2.5-VL usually uses Qwen2_5_VLForConditionalGeneration
        # If not available, fallback to AutoModel with trust_remote_code
        try:
...
class InternVLProcessorWrapper:
    ...
    def __call__(self, text=None, images=None, return_tensors="pt", **kwargs):
        ...
        output = {}
        # Separate tokenizer kwargs
        tokenizer_kwargs = {k: v for k, v in kwargs.items() if k in ["padding", "truncation", "max_length", "pad_to_multiple_of"]}
        
        if text is not None:
            # Tokenize text
            text_out = self.tokenizer(text, return_tensors=return_tensors, **tokenizer_kwargs)
            output.update(text_out)
        
        if images is not None:
            # Process images
            # InternVL often uses specific image processing (e.g. dynamic resolution)
            # But here we use standard CLIPImageProcessor as loaded
            # Note: InternVL usually expects 'pixel_values' of shape (N, C, H, W) or list of tensors
            # We'll just use the image_processor default behavior
            # Filter kwargs for image processor (it usually doesn't take padding/truncation)
            img_kwargs = {k: v for k, v in kwargs.items() if k not in tokenizer_kwargs}
            try:
                img_out = self.image_processor(images, return_tensors=return_tensors, **img_kwargs)
            except TypeError:
                 # If image processor doesn't accept kwargs, try without them
                img_out = self.image_processor(images, return_tensors=return_tensors)
            output.update(img_out)
...
class InternVLAdapter(CausalAdapter):
    def _load_processor(self):
        ...
        try:
            ...
            image_processor = AutoImageProcessor.from_pretrained(processor_id, trust_remote_code=True)
            return InternVLProcessorWrapper(tokenizer, image_processor)
        except Exception:
            # Fallback to AutoProcessor if wrapper fails (though unlikely if previous step failed)
            return AutoProcessor.from_pretrained(processor_id, trust_remote_code=True)
    ...
    def load_model(self) -> torch.nn.Module:
        ...
        # InternVL 2.5/2.0 often doesn't accept 'inputs_embeds' in forward, 
        # but PEFT/Trainer might pass it (even as None).
        # We monkey-patch forward to eat inputs_embeds.
        if not hasattr(model, "_original_forward"):
            model._original_forward = model.forward
            
            # Bound method handling
            import types
            def new_forward(self, *args, inputs_embeds=None, num_items_in_batch=None, **kwargs):
...
def build_adapter(model_cfg: ModelConfig, data_cfg: DataConfig) -> BaseAdapter:
    ...
    if model_type in {"internvl", "internvl2", "internvl3.5"}:
        return InternVLAdapter(model_cfg, data_cfg)
...
class LoRAAblationRunner:
    ...
    def _legacy_text_metrics(self, predictions: List[str], references: List[str]) -> Dict[str, float]:
    ...
    def _compute_text_metrics(
        ...
        *,
        ...
        sanitized_preds = [self._safe_str(pred) for pred in predictions]
        sanitized_refs = [self._safe_str(ref) for ref in references]

        valid_pairs = [
            (pred.strip(), ref.strip())
            for pred, ref in zip(sanitized_preds, sanitized_refs)
            if ref.strip()
        ]

        basic_stats: Dict[str, float] = {}
        if valid_pairs:
            preds_only = [p for p, _ in valid_pairs]
            refs_only = [r for _, r in valid_pairs]
            basic_stats["samples"] = float(len(valid_pairs))
            basic_stats["exact_match"] = float(sum(1 for p, r in valid_pairs if p == r) / len(valid_pairs))
            basic_stats["avg_pred_tokens"] = float(np.mean([len(p.split()) for p in preds_only]) if preds_only else 0.0)
            basic_stats["avg_ref_tokens"] = float(np.mean([len(r.split()) for r in refs_only]) if refs_only else 0.0)

        df = pd.DataFrame(
            {
                "prediction": sanitized_preds,
                "reference": sanitized_refs,
            }
        )

        timestamp = datetime.utcnow().strftime("%Y%m%d_%H%M%S")
        safe_prefix = "".join(ch if ch.isalnum() or ch in ("-", "_") else "_" for ch in (experiment_id or "eval"))
        try:
            from scripts.eval import calculate_evaluation_metrics  # type: ignore
        except Exception as exc:
            self.logger.warning(f"eval.py 메트릭 로드에 실패하여 기본 메트릭으로 대체합니다: {exc}")
            metrics = self._legacy_text_metrics(sanitized_preds, sanitized_refs)
        else:
            try:
                metrics = calculate_evaluation_metrics(df, exp_dir, timestamp, safe_prefix)
            except Exception as exc:
                self.logger.error(f"eval.py 메트릭 계산 실패: {exc}")
                metrics = self._legacy_text_metrics(sanitized_preds, sanitized_refs)
    ...
    def _run_generation_evaluation(
        ...
        *,
        ...
        pd.DataFrame(records).to_csv(predictions_csv_path, index=False)
        self.logger.info(f"예측 CSV 저장: {predictions_csv_path}")
        
        self.logger.info(f"예측 저장: {predictions_path}")

        metrics = self._compute_text_metrics(
            predictions,
            references,
            exp_dir=exp_dir,
