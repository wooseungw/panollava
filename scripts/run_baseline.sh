...
# ============================================================
# Usage:
#   bash scripts/run_baseline.sh                                           # all models
#   bash scripts/run_baseline.sh --max-tokens 256 --image-size 448 --max-length 1024
#   bash scripts/run_baseline.sh --models "qwen2-vl-2b,qwen25-vl-3b"      # subset
# ============================================================

# Use externally set GPU, default to 0
...
while [[ $# -gt 0 ]]; do
    case "$1" in
        ...
        -h|--help)
            ...
            echo "  --max-length N   Max input sequence length / truncation (default: 256)"
            echo "  --models LIST    Comma-separated model names to run (default: all)"
            echo "  -h, --help       Show this help message"
            echo ""
            echo "Available models:"
            echo "  qwen2-vl-2b       Qwen/Qwen2-VL-2B-Instruct       (model_type: qwen2_vl)"
            echo "  qwen25-vl-3b      Qwen/Qwen2.5-VL-3B-Instruct     (model_type: qwen_vl)"
            ...
            echo ""
            echo "Example:"
            echo "  bash scripts/run_baseline.sh --models 'qwen2-vl-2b,gemma3-4b'"
            exit 0
            ;;
...
# ============================================================
ALL_MODELS=(
    "qwen2-vl-2b|Qwen/Qwen2-VL-2B-Instruct|qwen2_vl|float16"
    "qwen25-vl-3b|Qwen/Qwen2.5-VL-3B-Instruct|qwen_vl|float16"
...
echo "============================================================"
echo "  Baseline Pipeline: QUIC-360 (Multi-Model)"
...
for entry in "${ALL_MODELS[@]}"; do
    ...
    MIXED_PREC="${MODEL_DTYPE/bfloat16/bf16}"
    MIXED_PREC="${MIXED_PREC/float16/fp16}"
    cat > "$CONFIG_PATH" <<EOF
